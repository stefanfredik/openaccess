...
<template>
  <Head title="Access Point Inventory" />
    ...
    <div class="flex flex-col gap-6 p-4 md:p-8">
      ...
      <Card class="overflow-hidden rounded-xl border-none bg-card shadow-none">
        <div class="flex items-center justify-between border-b border-border bg-card p-6">
          <h2 class="text-lg font-bold text-foreground">Daftar Inventori Access Point</h2>
          ...
          <Table>
            <TableHeader class="bg-muted/50">
              <TableRow class="border-b border-border hover:bg-transparent">
                ...
                <TableHead class="px-6 py-4 text-xs font-semibold tracking-wider text-muted-foreground uppercase">Frequency</TableHead>
            ...
            <TableBody>
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" class="w-48">
                        <DropdownMenuItem as-child class="cursor-pointer">
                          <Link :href="route('active-device.access-point.show', item.id)">
                            <FileText class="mr-2 h-4 w-4" />
                            Detail Lengkap
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem as-child class="cursor-pointer">
                          <Link :href="route('active-device.access-point.edit', item.id)">
                        ...
                        <div @click.stop>
                          <DeleteAction
                            :href="route('active-device.access-point.destroy', item.id)"
              ...
              <TableRow v-if="accessPoints.data.length === 0">
                <TableCell colspan="6" class="h-32 text-center text-muted-foreground italic"> No Access Points found in inventory. </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </CardContent>
      ...
      <Sheet :open="isDrawerOpen" @update:open="isDrawerOpen = $event">
        ...
        <SheetContent class="flex h-full flex-col border-none p-0 shadow-2xl sm:max-w-[500px]">
          <SheetHeader class="sr-only">
            <SheetTitle>Detail AP: {{ selectedAp?.name }}</SheetTitle>
            <SheetDescription>Koneksi, interface, dan layanan aktif pada perangkat ini.</SheetDescription>
          </SheetHeader>
          <DeviceDetailPreview v-if="selectedAp" :device="selectedAp" />
        </SheetContent>
      </Sheet>
    </div>
  </AppLayout>
