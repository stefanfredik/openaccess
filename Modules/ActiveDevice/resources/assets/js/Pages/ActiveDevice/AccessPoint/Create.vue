...
import { store, index as apIndex } from '@/routes/active-device/access-point';
...
const submit = () => {
    form.post('/active-devices/access-point');
};
...
<template>
    <Head title="Add Access Point" />

    <AppLayout :breadcrumbs="[
        { title: 'Access Points', href: apIndex().url },
        { title: 'Add AP', href: '#' }
    ]">
        <div class="flex flex-col gap-6 p-4 md:p-6 lg:p-8 max-w-4xl mx-auto">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Add Access Point</h1>
                    <p class="text-muted-foreground">Register a new Wireless Radio / Access Point.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Basic device identification and wireless details.
                        </CardDescription>
                    </CardHeader>
                    <CardContent class="grid gap-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <Input id="code" v-model="form.code" :class="{ 'border-destructive': form.errors.code }" placeholder="AP-XYZ" />
                                <p v-if="form.errors.code" class="text-sm text-destructive">{{ form.errors.code }}</p>
                            </div>

                            <div class="space-y-2">
                                <Label for="name">Name</Label>
                                <Input id="name" v-model="form.name" :class="{ 'border-destructive': form.errors.name }" placeholder="AP North Wing" />
                            ...
                            <div class="space-y-2">
                                ...
                                <Select v-model="form.frequency">
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="Dual Band">Dual Band</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <div class="space-y-2">
                                <Label for="ssid_count">SSID Count</Label>
                                <Input id="ssid_count" type="number" v-model="form.ssid_count" />
                            </div>
                        </div>
                    ...
                    <CardFooter class="flex justify-end gap-2 border-t p-6 mt-6">
                        <Button variant="outline" as-child>
                            <Link href="/active-devices/access-point">Cancel</Link>
                        </Button>
                        <Button type="submit" :disabled="form.processing">
                            {{ form.processing ? 'Saving...' : 'Save AP' }}
                        </Button>
                    </CardFooter>
                </Card>
            </form>
