  ...
  <AppLayout
    ...
    <div class="max-w-7xl space-y-6 p-4 md:p-6">
      ...
      <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div class="space-y-1">
          <div class="flex items-center gap-3">
            ...
            <Badge :variant="router.is_active ? 'default' : 'destructive'" class="capitalize">
              {{ router.is_active ? 'Active' : 'Inactive' }}
            </Badge>
          </div>
          <p class="flex items-center gap-2 text-sm text-muted-foreground">
            <span class="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-xs dark:bg-slate-800">{{ router.code }}</span>
            <span>&bull;</span>
      ...
      <div class="flex flex-col space-y-8 md:hidden">
        ...
        <!-- Service Ports -->
        <ServicePortsManager :device="router" device-type="Modules\ActiveDevice\Models\Router" />
