<script setup lang="ts">
  ...
  const submit = () => {
    form.post(route('active-device.olt.store'))
  }
...
<template>
  ...
  <AppLayout
    ...
    <div class="mx-4 max-w-4xl space-y-6 p-4 md:p-4">
      <!-- Header -->
      <div class="space-y-1">
        <h1 class="text-2xl font-bold tracking-tight">Tambah OLT</h1>
        <p class="text-sm text-foreground/60 text-muted-foreground">Silahkan lengkapi data perangkat OLT dibawah ini.</p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
          ...
          <CardContent class="grid gap-5">
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              <div class="space-y-1.5">
                ...
                <Input id="password" type="password" v-model="form.password" class="h-11 rounded-lg border-slate-200" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card class="border shadow-none">
          <CardHeader class="pb-4">
            <CardTitle class="text-base font-semibold">Spesifikasi & Lokasi</CardTitle>
            <CardDescription class="text-xs">Tahun pembelian, spesifikasi PON, dan koordinat.</CardDescription>
          </CardHeader>
          <CardContent class="grid gap-5">
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
              <div class="space-y-1.5">
        ...
        <Card class="border shadow-none">
          ...
          <CardContent class="grid gap-5">
            <div class="space-y-1.5">
              ...
              <FileUploader v-model="form.device_image" accept="image/png, image/jpeg, image/jpg" :max-size="5" :error="form.errors.device_image" />
            </div>
          ...
          <CardFooter class="flex justify-end gap-3 rounded-b-lg border-t bg-slate-50/50 p-6">
            ...
            <Button type="submit" :disabled="form.processing" class="rounded-lg bg-primary px-10 font-bold hover:bg-primary/90">
              {{ form.processing ? 'Menyimpan...' : 'Simpan OLT' }}
            </Button>
          </CardFooter>
        </Card>
      </form>
