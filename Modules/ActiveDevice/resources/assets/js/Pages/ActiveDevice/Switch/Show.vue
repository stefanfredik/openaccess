    ...
    <div class="max-w-7xl space-y-6 p-4 md:p-6">
      ...
      <div class="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div class="space-y-1">
          ...
          <p class="flex items-center gap-2 text-sm text-muted-foreground">
            <span class="rounded bg-muted px-1.5 py-0.5 font-mono text-[10px] uppercase">{{ networkSwitch.code }}</span>
            <span>&bull;</span>
            <span>{{ networkSwitch.brand }} {{ networkSwitch.model }}</span>
          </p>
        </div>
        <div class="flex items-center gap-2">
          ...
          <Button size="sm" as-child>
            <Link :href="route('active-device.switch.edit', networkSwitch.id)">
      ...
      <Tabs default-value="overview" class="hidden w-full flex-col gap-8 md:flex md:flex-row">
        ...
        <div class="flex-1">
          ...
          <!-- Tab 2: Service Ports -->
          <TabsContent v-if="networkSwitch.switch_type !== 'Unmanageable'" value="services" class="mt-0 space-y-6">
            <ServicePortsManager :device="networkSwitch" device-type="Modules\ActiveDevice\Models\AdSwitch" />
          </TabsContent>
