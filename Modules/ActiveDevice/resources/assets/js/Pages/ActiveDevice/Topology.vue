<script setup lang="ts">
  ...
  const flattenTopology = (items: any[], level = 0, parentUid = '') => {
    items.forEach((item) => {
      ...
      if (parentUid) {
        const edgeId = showAllLines.value ? `${parentUid}-${uid}-${item.source_port}-${item.destination_port}` : `${parentUid}-${uid}`
  ...
  <AppLayout :breadcrumbs="[{ title: 'Topology Map', href: '#' }]">
    <div class="relative h-[calc(100vh-64px)] overflow-hidden bg-[#0a0b0f] select-none">
      ...
      <div class="absolute top-6 left-6 z-50 flex flex-col gap-4">
        <div class="flex gap-2">
          <div class="flex gap-1 rounded-lg border border-white/10 bg-black/60 p-1 shadow-2xl backdrop-blur-md">
            <button @click="handleZoom(0.1)" class="rounded p-2 text-white/60 transition-colors hover:bg-white/10 hover:text-white" title="Zoom In">
              <ZoomIn class="h-5 w-5" />
            </button>
            <button @click="handleZoom(-0.1)" class="rounded p-2 text-white/60 transition-colors hover:bg-white/10 hover:text-white" title="Zoom Out">
          ...
          <div class="flex gap-1 rounded-lg border border-white/10 bg-black/60 p-1 shadow-2xl backdrop-blur-md">
            <button
              ...
              :class="[
                ...
                isLocked ? 'bg-red-500/80 text-white shadow-lg' : 'text-white/40 hover:bg-white/5 hover:text-white',
              ]"
      ...
      <div
        ...
        <svg width="2000" height="2000" class="overflow-visible" :style="transformStyle">
          ...
          <g
            ...
            <!-- Status Glow -->
            <circle :cx="node.x + 80" :cy="node.y" r="5" :fill="getStatusColor(node.status)" class="animate-pulse shadow-[0_0_10px_currentColor]" />

            <!-- Icon Area -->
            <foreignObject :x="node.x + 20" :y="node.y + 10" width="40" height="40">
              <div class="flex h-full w-full items-center justify-center text-white transition-colors group-hover:text-primary">
      ...
      <div v-if="nodes.length === 0" class="absolute inset-0 flex flex-col items-center justify-center text-white/40">
        <div class="mb-6 rounded-full border border-white/5 bg-white/5 p-8">
      ...
      <div
        ...
        <div class="mb-1 font-bold tracking-widest text-white/30 uppercase">Visualization Info</div>
        <div class="flex items-center gap-2">
          <span class="h-0.5 w-3 border border-dashed border-white/20 bg-white/10"></span>
          Link established
        </div>
        <div class="flex items-center gap-2">
          <div class="h-1.5 w-1.5 animate-pulse rounded-full bg-primary shadow-[0_0_10px_rgba(79,70,229,0.5)]"></div>
    ...
    <Sheet v-model:open="isSheetOpen">
      <SheetContent class="w-[400px] border-l border-white/10 bg-[#0a0b0f] p-6 text-white sm:w-[540px]">
        <SheetHeader class="mb-6">
          <SheetTitle class="flex items-center gap-3 text-xl text-white">
            <component :is="getIcon(selectedNode?.type || '')" class="h-6 w-6 text-blue-400" />
            {{ selectedNode?.name }}
          </SheetTitle>
          <SheetDescription class="text-white/60"> Detailed information for this active device. </SheetDescription>
        </SheetHeader>

        <div v-if="selectedNode" class="space-y-6">
          <div v-if="isLoadingDetails" class="flex flex-col items-center justify-center gap-3 py-12 text-white/50">
            <Activity class="h-8 w-8 animate-spin text-primary" />
            <span class="text-xs font-medium tracking-widest uppercase">Loading Details...</span>
          </div>

          <template v-else-if="nodeDetails">
            <div class="space-y-1">
              <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Status</div>
              <div class="flex items-center gap-2">
                <div
            ...
            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-1">
                <div class="text-xs font-bold tracking-widest text-white/40 uppercase">IP Address</div>
                <div class="font-mono font-bold tracking-wide text-blue-400">
                  {{ nodeDetails.ip_address || 'N/A' }}
                </div>
              </div>
              <div class="space-y-1">
                <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Device Type</div>
                <div class="font-medium">
                  {{ nodeDetails.type }}
                </div>
              </div>
              <div class="space-y-1">
                <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Brand</div>
                <div class="font-medium">
                  {{ nodeDetails.brand || 'Unknown' }}
                </div>
              </div>
              <div class="space-y-1">
                <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Model</div>
                <div class="font-medium">
                  {{ nodeDetails.model || 'Unknown' }}
                </div>
              </div>
              <div class="space-y-1">
                <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Area</div>
                <div class="font-medium">
                  {{ nodeDetails.area_name || '-' }}
                </div>
              </div>
              <div class="space-y-1">
                <div class="text-xs font-bold tracking-widest text-white/40 uppercase">POP</div>
                <div class="font-medium">
            ...
            <div class="space-y-1">
              <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Hardware Code</div>
              <div class="rounded border border-white/10 bg-white/5 p-3 font-mono text-sm select-all">
                {{ nodeDetails.code }}
              </div>
            </div>

            <div v-if="nodeDetails.description" class="space-y-1">
              <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Description</div>
              <div class="rounded border border-white/10 bg-white/5 p-3 text-sm text-white/60">
                {{ nodeDetails.description }}
              </div>
            </div>

            <div class="grid grid-cols-2 gap-4 border-t border-white/10 pt-4 text-xs text-white/40">
              <div>
                Serial:
                <span class="font-mono text-white/60">{{ nodeDetails.serial_number || 'N/A' }}</span>
              </div>
              <div>
                Ports:
                <span class="font-mono text-white/60">{{ nodeDetails.port_count || 'N/A' }}</span>
              </div>
            </div>

            <div class="space-y-2 border-t border-white/10 pt-4">
              <div class="text-xs font-bold tracking-widest text-white/40 uppercase">Connected Ports</div>
                ...
                <table class="w-full text-left text-sm">
                  <thead>
                    <tr class="border-b border-white/10 text-xs text-white/40">
                      ...
                      <th class="py-2 text-right font-medium">Type</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-white/5">
                    <tr v-for="conn in nodeDetails.connections" :key="conn.id" class="group/row">
                      <td class="py-2 font-mono text-blue-400">
                        {{ conn.local_port }}
                      </td>
                      <td class="py-2 font-medium text-white/80">
                        {{ conn.remote_device }}
                      </td>
                      <td class="py-2 font-mono text-white/60">
                        {{ conn.remote_port }}
                      </td>
                      <td class="py-2 text-right">
                        <span class="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] tracking-tighter text-white/40 uppercase">
                          {{ conn.type }}
                        </span>
                      </td>
                    </tr>
              ...
              <div v-else class="py-2 text-sm text-white/30 italic">No active connections found.</div>
            </div>
          </template>
        </div>
      </SheetContent>
