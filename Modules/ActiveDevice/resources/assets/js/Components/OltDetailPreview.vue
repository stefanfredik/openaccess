<script setup lang="ts">
  import { ArrowRightLeft, CheckCircle2, LockKeyhole, Monitor, PencilLine, PlusCircle, Zap } from 'lucide-vue-next'
...
<template>
  <div class="flex h-full flex-col">
    <div class="border-b bg-slate-50 p-6">
      ...
      <p class="text-sm text-slate-500 italic">OLT - {{ olt.brand }} {{ olt.model }}</p>
    </div>

    <Tabs default-value="services" class="flex flex-1 flex-col">
      <TabsList class="h-12 w-full justify-start rounded-none border-b bg-white px-6">
        <TabsTrigger
          value="services"
      ...
      <div class="flex-1 overflow-y-auto">
        <TabsContent value="services" class="m-0 space-y-4 p-6">
          ...
          <div class="grid grid-cols-1 gap-3">
            ...
            <div v-if="activeServicePorts.length === 0" class="py-8 text-center text-sm text-muted-foreground italic">No active services found.</div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="interfaces" class="m-0 p-6">
          ...
          <div v-if="!isInterfaceManaging">
            ...
            <div class="grid grid-cols-4 gap-2 sm:grid-cols-6">
              <div
                ...
                :title="inf.status === 'up' ? 'Connected' : inf.status === 'error' ? 'Error / Down' : 'Idle'">
                <Zap v-if="inf.type === 'Fiber'" class="mb-1 h-4 w-4" />
          ...
          <div v-else>
            <div class="mb-4">
              <Button variant="ghost" size="sm" @click="isInterfaceManaging = false" class="h-8"> ← Kembali ke Visualisasi </Button>
            </div>
            <InterfaceManager :olt-id="olt.id" :olt-type="'Modules\\ActiveDevice\\Models\\Olt'" :interfaces="activeInterfaces" />
          </div>
        </TabsContent>

        <TabsContent value="topology" class="m-0 p-6">
          <h3 class="mb-6 text-xs font-bold tracking-widest text-gray-400 uppercase">Active Links</h3>

          <div class="relative space-y-6">
            <div class="absolute top-2 bottom-2 left-[7px] w-0.5 border-l-2 border-dashed border-blue-100"></div>

            <div v-for="conn in sourceConnections" :key="conn.id" class="relative pl-8">
              <div class="absolute top-1 left-0 h-4 w-4 rounded-full border-2 border-white bg-blue-600 shadow-sm"></div>
              <div class="rounded-lg border border-blue-100 bg-blue-50 p-4">
                <div class="mb-2 flex items-center justify-between">
                  <span class="rounded bg-blue-600 px-2 py-0.5 text-[10px] font-bold tracking-tight text-white uppercase">
                ...
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <p class="text-sm font-bold text-slate-800">
                      {{ conn.destination?.name || 'Unknown' }}
                    </p>
                    <p class="text-xs text-slate-500">
                      Target Port:
                      <span class="font-mono text-blue-600">{{ conn.destination_port }}</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div v-if="sourceConnections.length === 0" class="py-8 text-center text-sm text-muted-foreground italic">No active links detected.</div>

            <Button variant="outline" class="mt-4 w-full border-dashed py-6 text-gray-400 hover:border-blue-300 hover:text-blue-600">
          ...
          <div class="overflow-hidden rounded-lg border border-slate-100 bg-white shadow-sm">
            <table class="w-full text-left text-sm">
              <tbody class="divide-y divide-gray-50">
                <tr v-for="(val, key) in deviceDetails" :key="key" class="transition-colors hover:bg-slate-50/50">
                  <td class="w-1/3 border-r border-slate-50/50 bg-slate-50/30 px-4 py-3 font-medium text-slate-500">
                    {{ key }}
                  </td>
