    ...
    <Card class="overflow-hidden">
      <div class="flex flex-col md:flex-row">
        <div class="relative flex aspect-video w-full items-center justify-center bg-muted/30 md:w-1/3 lg:w-1/4">
          <img v-if="olt.device_image" :src="'/storage/' + olt.device_image" alt="Device Image" class="h-full w-full object-cover" />
          ...
          <div class="mb-6 grid grid-cols-2 gap-6 md:grid-cols-4">
            <div class="space-y-1">
              ...
              <div class="flex items-center gap-2">
                <div class="h-2.5 w-2.5 rounded-full" :class="olt.status === 'Active' ? 'bg-green-500' : 'bg-red-500'"></div>
                <span class="font-medium">{{ olt.status }}</span>
              </div>
            </div>
            <div class="space-y-1">
            ...
            <div class="col-span-2 space-y-1">
              <span class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Service Status</span>
              <div class="flex flex-wrap gap-2">
                <Badge variant="secondary" v-for="status in olt.service_status" :key="status" class="h-5 px-2 py-0 text-xs">
                  {{ status }}
                </Badge>
      ...
      <CardContent>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
          ...
          <div class="space-y-1">
            ...
            <div class="font-mono text-sm text-muted-foreground">********</div>
          </div>
        </div>
      </CardContent>
    </Card>
