<script setup lang="ts">
  import { ArrowRightLeft, CheckCircle2, Info, LockKeyhole, Monitor, Network, PencilLine, ShieldCheck, Zap } from 'lucide-vue-next'
  ...
  const activeServicePorts = computed(() => props.device.service_ports || props.device.servicePorts || [])
  const sourceConnections = computed(() => props.device.source_connections || props.device.sourceConnections || [])
  const destinationConnections = computed(() => props.device.destination_connections || props.device.destinationConnections || [])
  ...
  const ethernetInterfaces = computed(() => {
    return activeInterfaces.value.filter((i: any) => (i.type === 'Ethernet' || i.type === 'Gigabit') && !i.name.toUpperCase().includes('PON'))
  })

  const opticalInterfaces = computed(() => {
    return activeInterfaces.value.filter((i: any) => (i.type === 'Fiber' || i.type === 'Optical') && !i.name.toUpperCase().includes('PON'))
  })

  const ponInterfaces = computed(() => {
    return activeInterfaces.value.filter((i: any) => i.type === 'PON' || i.name.toUpperCase().includes('PON'))
  })
  ...
  const deviceType = computed(() => {
    ...
    if (props.device.switch_type) return 'Modules\\ActiveDevice\\Models\\AdSwitch'
    if (props.device.type && props.device.brand) {
      // Generic check for AP/CPE if we can't be sure, but usually namespaces help
      if (props.device.type.includes('AP') || props.device.type.includes('Access')) return 'Modules\\ActiveDevice\\Models\\AccessPoint'
    }
    // Check for CPE specific fields or breadcrumbs context if possible,
    // but here we rely on the object structure
    if (props.device.model && !props.device.pon_type && !props.device.switch_type) {
  ...
  const deviceDetails = computed(() => {
    const details: Record<string, any> = {
      ...
      Status: props.device.status || (props.device.is_active ? 'Active' : 'Inactive'),
...
<template>
  <div class="flex h-full flex-col">
    ...
    <Tabs default-value="detail" class="flex flex-1 flex-col">
      <TabsList class="h-12 w-full justify-start space-x-2 rounded-none border-b bg-background px-6">
        <TabsTrigger
          value="detail"
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <TabsContent value="detail" class="m-0 p-6">
          <h3 class="mb-4 text-xs font-bold tracking-widest text-gray-400 uppercase">Device Information</h3>
          ...
          <div class="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
            <table class="w-full text-left text-sm">
              <tbody class="divide-y divide-border/50">
                <tr v-for="(val, key) in deviceDetails" :key="key" class="transition-colors hover:bg-muted/30">
                  <td class="w-1/3 border-r border-border/50 bg-muted/20 px-4 py-3 font-medium text-muted-foreground">
                    {{ key }}
                  </td>
        ...
        <!-- Services Tab -->
        <TabsContent v-if="device.switch_type !== 'Unmanageable'" value="services" class="m-0 space-y-4 p-6">
          <h3 class="text-xs font-bold tracking-widest text-gray-400 uppercase">Active Services</h3>
            ...
            <div
              ...
              <div class="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p :class="['text-xs', port.status === 'Active' ? 'text-green-500' : 'text-gray-400']">
                    Status:
                    {{ port.status === 'Active' ? 'Running' : 'Closed' }}
                  </p>
                </div>
              </div>
              <CheckCircle2 v-if="port.status === 'Active'" class="h-5 w-5 text-green-500" />
              <LockKeyhole v-else class="h-5 w-5 text-gray-300" />
            </div>
            <div v-if="activeServicePorts.length === 0" class="py-8 text-center text-sm text-muted-foreground italic">
        ...
        <TabsContent value="interfaces" class="m-0 p-6">
          <h3 class="mb-4 text-xs font-bold tracking-widest text-gray-400 uppercase">Physical Ports</h3>

          <div v-if="!isInterfaceManaging">
            <div class="mb-4 flex justify-end">
              <Button variant="outline" size="sm" @click="isInterfaceManaging = true" class="h-8">
                <PencilLine class="mr-2 h-4 w-4" />
                Manage Ports
              </Button>
            </div>

            <div v-if="activeInterfaces.length > 0" class="space-y-6">
              <!-- Ethernet Ports -->
              <div v-if="ethernetInterfaces.length > 0">
                <h4 class="mb-3 text-xs font-bold tracking-widest text-slate-500 uppercase">Ethernet (RJ45)</h4>
                <div class="grid grid-cols-4 gap-2 sm:grid-cols-5">
                  <div
                    ...
                    :title="inf.name + ' - ' + (inf.status === 'up' ? 'Connected' : 'Idle')">
                    <!-- Status Dot -->
                    <div v-if="inf.status === 'up'" class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-green-500"></div>
                    <Monitor class="mb-1 h-5 w-5 opacity-80" />
                    <span class="text-[10px] font-bold tracking-tighter uppercase">{{ inf.name }}</span>
                  </div>
                </div>
              </div>
              ...
              <div v-if="opticalInterfaces.length > 0">
                <h4 class="mb-3 text-xs font-bold tracking-widest text-slate-500 uppercase">Optical (SFP/SFP+)</h4>
                <div class="grid grid-cols-4 gap-2 sm:grid-cols-5">
                  <div
                    ...
                    :title="inf.name + ' - ' + (inf.status === 'up' ? 'Connected' : 'Idle')">
                    <!-- Status Dot -->
                    <div v-if="inf.status === 'up'" class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-blue-500"></div>
                    <div v-else-if="inf.status === 'error'" class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-red-500"></div>
              ...
              <div v-if="ponInterfaces.length > 0">
                <h4 class="mb-3 text-xs font-bold tracking-widest text-slate-500 uppercase">PON Ports</h4>
                <div class="grid grid-cols-4 gap-2 sm:grid-cols-5">
                  <div
                    ...
                    :title="inf.name + ' - ' + (inf.status === 'up' ? 'Connected' : 'Idle')">
                    <!-- Status Dot -->
                    <div v-if="inf.status === 'up'" class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-orange-500"></div>
                    <div v-else-if="inf.status === 'error'" class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-red-500"></div>
            ...
            <div v-else class="py-8 text-center text-sm text-muted-foreground italic">No physical interfaces found.</div>
          </div>

          <div v-else>
            <div class="mb-4">
              <Button variant="ghost" size="sm" @click="isInterfaceManaging = false" class="h-8"> ← Back to Visualization </Button>
            </div>
            <InterfaceManager :device-id="device.id" :device-type="deviceType" :interfaces="activeInterfaces" />
          </div>
        </TabsContent>

        <!-- Topology/Links Tab -->
        <TabsContent value="topology" class="m-0 p-6">
          <h3 class="mb-6 text-xs font-bold tracking-widest text-gray-400 uppercase">Active Connections</h3>

          <div class="relative space-y-6">
            <div class="absolute top-2 bottom-2 left-[7px] w-0.5 border-l-2 border-dashed border-border"></div>

            <div v-for="conn in allConnections" :key="conn.id" class="relative pl-8">
              <div
                :class="[
                  'absolute top-1 left-0 h-4 w-4 rounded-full border-2 border-white shadow-sm',
                  conn.role === 'Source' ? 'bg-blue-600' : 'bg-green-600',
                ]"></div>
              <div class="rounded-lg border border-border bg-muted/40 p-4">
                ...
                <div class="flex items-center space-x-3">
                  <ArrowRightLeft class="h-5 w-5 text-blue-400" />
                  <div>
                    <p class="text-sm font-bold text-foreground">
                      {{ conn.peer?.name || 'Unknown Device' }}
                    </p>
            ...
            <div v-if="allConnections.length === 0" class="py-8 text-center text-sm text-muted-foreground italic">No active connections.</div>

            <!-- Note: Adding connections usually happens on the details page, so we just show status here -->
            <div class="mt-4 text-center">
              <p class="text-xs text-muted-foreground">To add or manage connections, visit the full device details page.</p>
            </div>
          </div>
        </TabsContent>
      </div>
