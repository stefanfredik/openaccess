          ...
          <div class="mb-6 grid grid-cols-2 gap-6 md:grid-cols-4">
            <div class="space-y-1">
              ...
              <div class="flex items-center gap-2">
                <div class="h-2.5 w-2.5 rounded-full" :class="networkSwitch.is_active ? 'bg-green-500' : 'bg-red-500'"></div>
                <span class="font-medium">{{ networkSwitch.is_active ? 'Active' : 'Inactive' }}</span>
              </div>
            </div>
            <div class="space-y-1">
            ...
            <div class="space-y-1">
              <span class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Port Count</span>
              <div class="font-medium">{{ networkSwitch.port_count }} Ports</div>
            </div>
            <div class="space-y-1">
              <span class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Switch Type</span>
      ...
      <CardContent>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
          <div class="space-y-1">
            ...
            <div class="font-medium">{{ networkSwitch.brand }} {{ networkSwitch.model }}</div>
          </div>
          <div class="space-y-1">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center gap-2 font-mono text-sm">
              <span>{{ networkSwitch.latitude || '0' }}, {{ networkSwitch.longitude || '0' }}</span>
              <a
                v-if="networkSwitch.latitude && networkSwitch.longitude"
                :href="`https://www.google.com/maps/search/?api=1&query=${networkSwitch.latitude},${networkSwitch.longitude}`"
                target="_blank"
    ...
    <Card>
      ...
      <CardContent>
        <p class="text-sm leading-relaxed text-foreground/80">
          {{ networkSwitch.description || 'Tidak ada keterangan tambahan.' }}
        </p>
      </CardContent>
    </Card>
  </div>
