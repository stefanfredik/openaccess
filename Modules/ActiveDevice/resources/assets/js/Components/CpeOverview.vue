...
<template>
  <div class="space-y-6">
          ...
          <div class="mb-6 grid grid-cols-2 gap-6 md:grid-cols-4">
            <div class="space-y-1">
              ...
              <div class="flex items-center gap-2">
                <div class="h-2.5 w-2.5 rounded-full" :class="cpe.status === 'Active' ? 'bg-green-500' : 'bg-red-500'"></div>
                <span class="font-medium">{{ cpe.status || 'Inactive' }}</span>
              </div>
            </div>
            <div class="space-y-1">
      ...
      <CardContent>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
          <div class="space-y-1">
            ...
            <div class="font-medium">{{ cpe.brand }} {{ cpe.model }}</div>
          </div>
          <div class="space-y-1">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center gap-2 font-mono text-sm">
              <span>{{ cpe.latitude || '0' }}, {{ cpe.longitude || '0' }}</span>
              <a
    ...
    <Card v-if="cpe.active_device">
      <CardHeader class="pb-3">
        <CardTitle class="text-sm font-bold tracking-wider text-muted-foreground uppercase">Uplink Device</CardTitle>
      </CardHeader>
      <CardContent>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div class="space-y-1">
            <span class="text-xs text-muted-foreground">Name / Code</span>
            <div class="font-medium">{{ cpe.active_device.name }} ({{ cpe.active_device.code }})</div>
          </div>
          <div class="space-y-1">
