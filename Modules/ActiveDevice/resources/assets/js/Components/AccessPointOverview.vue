          ...
          <div class="mb-6 grid grid-cols-2 gap-6 md:grid-cols-4">
            <div class="space-y-1">
              ...
              <div class="flex items-center gap-2">
                <div class="h-2.5 w-2.5 rounded-full" :class="accessPoint.is_active ? 'bg-green-500' : 'bg-red-500'"></div>
                <span class="font-medium">{{ accessPoint.is_active ? 'Active' : 'Inactive' }}</span>
              </div>
            </div>
            <div class="space-y-1">
            ...
            <div class="space-y-1">
              <span class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Frequency</span>
              <div class="font-medium">
                {{ accessPoint.frequency || '-' }}
              </div>
            </div>
            <div class="space-y-1">
              <span class="text-xs font-bold tracking-wider text-muted-foreground uppercase">SSID Count</span>
              <div class="font-medium">
                {{ accessPoint.ssid_count || 0 }}
              </div>
            </div>
          </div>
        </div>
      ...
      <CardContent>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
          <div class="space-y-1">
            ...
            <div class="font-medium">{{ accessPoint.brand }} {{ accessPoint.model }}</div>
          </div>
          <div class="space-y-1">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <div class="space-y-1">
            ...
            <div class="flex items-center gap-2 font-mono text-sm">
              <span>{{ accessPoint.latitude || '0' }}, {{ accessPoint.longitude || '0' }}</span>
              <a
                v-if="accessPoint.latitude && accessPoint.longitude"
                :href="`https://www.google.com/maps/search/?api=1&query=${accessPoint.latitude},${accessPoint.longitude}`"
                target="_blank"
    ...
    <Card>
      ...
      <CardContent>
        <p class="text-sm leading-relaxed text-foreground/80">
          {{ accessPoint.description || 'Tidak ada keterangan tambahan.' }}
        </p>
      </CardContent>
    </Card>
  </div>
