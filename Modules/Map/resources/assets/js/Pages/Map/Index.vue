...
const selectedAreaId = ref<string>(
    localStorage.getItem('map_selected_area') || 'all',
);
    ...
    try {
        const url =
            selectedAreaId.value === 'all'
                ? route('map.data')
                : route('map.data', { area_id: selectedAreaId.value });
        ...
        if (selectedAreaId.value !== 'all') {
            ...
            const area = props.areas.find(
                (a) => a.id === parseInt(selectedAreaId.value),
            );
            ...
            mapData.value.features.length > 0
        ) {
        ...
        if (isDrawingCable.value) {
            ...
            if (!drawPolyline) {
                ...
                drawPolyline.setLatLngs(
                    pendingPath.value as L.LatLngExpression[],
                );
            }
            redrawEditMarkers();
            return;
        ...
        if (layer instanceof L.GeoJSON) {
            layer.eachLayer((marker: any) => {
                if (
                    ...
                    marker.feature?.properties?.type === type
                ) {
                    targetMarker = marker;
                }
            });
...
window.editCable = (id: number) => {
        ...
        if (layer instanceof L.GeoJSON) {
            layer.eachLayer((l: any) => {
                if (
                    ...
                    l.feature?.properties?.type === 'cable'
                ) {
    ...
    if (targetFeature && targetLayer) {
        ...
        const coords = targetFeature.geometry.coordinates.map((c: any) => [
            c[1],
            c[0],
        ]);
        ...
        map.closePopup();
        toast.info(
            `Mengedit jalur kabel "${targetFeature.properties.name}". Geser titik putih untuk ubah jalur. Klik kanan titik untuk hapus.`,
            {
                position: 'top-center',
                duration: 5000,
            },
...
watch(pendingDeviceType, (newVal) => {
    if (newVal === 'cable') {
        ...
        toast.info(
            'Mode Gambar Kabel: Klik di peta untuk membuat jalur. Klik "Selesai" untuk menyimpan.',
            {
                duration: 5000,
                position: 'top-center',
            },
...
<template>
    <AppLayout
        :breadcrumbs="[{ title: 'Network Map', href: route('map.index') }]"
    >
        ...
        >
            ...
            <div class="absolute bottom-6 left-4 z-[1000]">
                <div
                    class="rounded-full border border-blue-100 bg-white p-1 shadow-xl transition-transform hover:scale-110"
                >
                    <DropdownMenu>
                        ...
                        >
                            <DropdownMenuLabel
                                >Add New Device</DropdownMenuLabel
                            >
                            <DropdownMenuSeparator />
                            <DropdownMenuGroup>
                                <DropdownMenuLabel
                                    class="py-1 text-[10px] font-bold text-muted-foreground uppercase"
                                    >Active Devices</DropdownMenuLabel
                                >
                                <DropdownMenuItem
                            ...
                            <DropdownMenuGroup>
                                <DropdownMenuLabel
                                    class="py-1 text-[10px] font-bold text-muted-foreground uppercase"
                                    >Passive Devices</DropdownMenuLabel
                                >
                                <DropdownMenuItem
        ...
        >
            <div
                class="w-full max-w-sm animate-in rounded-xl bg-white p-6 shadow-2xl duration-200 zoom-in-95"
            >
