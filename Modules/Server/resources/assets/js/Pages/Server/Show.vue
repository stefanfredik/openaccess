<script setup lang="ts">
  ...
  const allConnections = computed(() => {
    ...
    selectedRack.value.contents.forEach((content: any) => {
      ...
      if (!device) return // Add source connections
              ...
              <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div class="flex flex-wrap items-center gap-2">
                  <template v-for="rack in server.racks" :key="rack.id">
                    <Button :variant="selectedRackId === rack.id ? 'default' : 'outline'" size="sm" @click="selectedRackId = rack.id">
                ...
                <div v-if="selectedRack" class="flex items-center gap-2">
                  ...
                  <DeleteAction
                    :href="route('racks.destroy', selectedRack.id)"
                    title="Delete Rack"
                    description="Are you sure you want to delete this rack? All mounted devices will be unlinked.">
                    <template #trigger>
              ...
              <div v-if="selectedRack" class="grid gap-6 lg:grid-cols-3">
                      ...
                      <div class="relative mx-auto w-full rounded-t-lg border-x-[12px] border-t-[12px] border-slate-800 bg-slate-900 shadow-2xl">
                        ...
                        <div ref="rackContainerRef" class="flex flex-col-reverse gap-px bg-slate-800 overflow-hidden relative">
                          ...
                          <div v-for="u in selectedRack.u_capacity" :key="u" class="group relative flex h-[40px] w-full items-center bg-slate-900/50">
                            <!-- U Number -->
                            <div
                              class="flex w-10 h-full items-center justify-center border-r border-slate-800 text-[10px] font-bold text-slate-500 bg-slate-950/40">
                              {{ u }}
                            </div>
                                ...
                                <TooltipProvider>
                                  <Tooltip>
                                      ...
                                      <div
                                        :ref="
                                          (el) => {
                                            if (el)
                                              deviceRefs.set(
                                                getDeviceAtU(selectedRack, u).device_type + ':' + getDeviceAtU(selectedRack, u).device_id,
                                                el as HTMLElement,
                                              )
                                          }
                                        "
                                        ...
                                        :class="[
                                          ...
                                          'border-white/20',
                                          activeDeviceId === getDeviceAtU(selectedRack, u).device_id &&
                                          activeDeviceType === getDeviceAtU(selectedRack, u).device_type
                                            ? 'ring-2 ring-white ring-inset brightness-125'
                                            : '',
                                        ]"
                                        :style="{
                                          height: `calc(${getDeviceAtU(selectedRack, u).unit_size} * 40px + (${getDeviceAtU(selectedRack, u).unit_size} - 1) * 1px)`,
                                        }">
                                        <div class="flex flex-col min-w-0 z-10">
                                          <span class="truncate text-[11px] font-bold leading-tight">{{
                                            getDeviceAtU(selectedRack, u).device?.name
                                          }}</span>
                                        ...
                                        <div class="absolute right-12 inset-y-1 flex items-center justify-end pointer-events-none">
                                          <div
                                            class="bg-black/10 backdrop-blur-[2px] rounded border border-white/10 p-1.5 flex items-center justify-center h-[calc(100%-8px)] my-auto">
                                            <div
                                              class="grid grid-flow-col gap-x-1.5 gap-y-1"
                                              :style="{
                                              ...
                                              <template v-if="getDeviceAtU(selectedRack, u).device?.interfaces?.length">
                                                <div
                                                  v-for="iface in getDeviceAtU(selectedRack, u).device.interfaces"
                                                  :key="iface.id"
                                                  :ref="
                                                    (el) => {
                                                      if (el) portRefs.set(`interface:${iface.id}`, el as HTMLElement)
                                                    }
                                                  "
                                                  :title="iface.name"
                                                  class="h-[5px] w-[5px] rounded-full bg-white/70 shadow-[0_0_4px_rgba(255,255,255,0.4)] ring-[0.5px] ring-white/20 transition-all hover:bg-white hover:scale-125"></div>
                                              </template>
                                              <template v-else>
                                                <div
                                                  v-for="p in getDeviceAtU(selectedRack, u).device?.port_count || 0"
                                                  :key="p"
                                                  class="h-[5px] w-[5px] rounded-full bg-white/70 shadow-[0_0_4px_rgba(255,255,255,0.4)] ring-[0.5px] ring-white/20 transition-all hover:bg-white hover:scale-125"></div>
                                              </template>
                                            </div>
                                    ...
                                    <TooltipContent
                                      side="right"
                                      :side-offset="15"
                                      class="w-64 p-0 overflow-hidden border-slate-800 bg-slate-950 shadow-2xl">
                                      ...
                                      <div class="p-4 space-y-3">
                                        <div class="flex items-center justify-between border-b border-white/10 pb-2">
                                          ...
                                          <Badge variant="outline" class="text-[10px] h-4 border-white/20 text-white">{{
                                            getDeviceAtU(selectedRack, u).device_type.split('\\').pop()
                                          }}</Badge>
                                        </div>
                                        ...
                                        <div class="space-y-2">
                                          ...
                                          <div class="grid grid-cols-2 gap-2">
                                            <div class="flex flex-col">
                                              ...
                                              <span class="text-xs font-medium text-slate-200">{{
                                                getDeviceAtU(selectedRack, u).device?.model || '-'
                                              }}</span>
                                            </div>
                                            <div class="flex flex-col">
                                              <span class="text-[10px] text-slate-400 uppercase">Code</span>
                                              <span class="text-xs font-medium text-slate-200">{{
                                                getDeviceAtU(selectedRack, u).device?.code || '-'
                                              }}</span>
                                            </div>
                                          </div>
                                          ...
                                          <div class="flex flex-col border-t border-white/5 pt-2">
                                            ...
                                            <span class="text-xs font-medium text-slate-200"
                                              >{{ getDeviceAtU(selectedRack, u).device?.port_count || 0 }} Total Ports</span
                                            >
                                          </div>
                                          ...
                                          <div class="flex flex-col border-t border-white/5 pt-2">
                                            ...
                                            <span class="text-xs font-medium text-slate-200"
                                              >U{{ getDeviceAtU(selectedRack, u).unit_start }} ({{
                                                getDeviceAtU(selectedRack, u).unit_size
                                              }}
                                              Unit)</span
                                            >
                                          </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                  ...
                  <div class="grid gap-1">
                    <Card
                      v-for="item in selectedRack.contents"
                      :key="item.id"
                      class="group overflow-hidden transition-all hover:border-primary/50 shadow-sm border-slate-100 dark:border-white/5">
                          ...
                          <div class="flex items-center gap-3">
                            ...
                            <div>
                              <div class="font-semibold text-sm leading-tight flex items-center gap-2">
                                ...
                                <Badge
                                  variant="outline"
                                  class="text-[10px] h-4 px-1.5 font-normal opacity-70 border-slate-200 dark:border-white/10"
                                  >{{ item.device_type.split('\\').pop() }}</Badge
                                >
                              </div>
                              <div class="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                                <span class="font-mono bg-muted/70 px-1.5 rounded text-primary"
                                  >U{{ item.unit_start }}-{{ item.unit_start - item.unit_size + 1 }}</span
                                >
                    ...
                    <div
                      v-if="!selectedRack.contents?.length"
                      class="flex h-40 flex-col items-center justify-center rounded-lg border-2 border-dashed bg-muted/20">
          ...
          <div class="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-6 py-6">
            ...
            <div class="hidden md:flex flex-col">
              ...
              <div class="flex-1 border-2 border-slate-800 rounded-lg bg-slate-950 p-1.5 overflow-y-auto max-h-[350px] scrollbar-thin">
                <div class="flex flex-col-reverse gap-px bg-slate-800 overflow-hidden">
                  <div
                    v-for="u in selectedRack?.u_capacity"
                    :key="u"
                    ...
                    <div class="flex-1 h-full relative">
                      <!-- Proposed Selection -->
                      <div
                        v-if="u >= mountForm.unit_start && u < mountForm.unit_start + mountForm.unit_size"
                        :class="[
                      ...
                      <!-- Existing Content -->
                      <div
                        v-if="isUSlotOccupied(selectedRack, u)"
                        class="absolute inset-x-0 inset-y-0.5 z-10 bg-slate-800/80 border-x border-slate-700 flex items-center px-1">
            ...
            <div class="flex flex-col h-full">
              ...
              <div v-else-if="availableDevices.length === 0" class="flex flex-col items-center justify-center flex-1 py-12 text-center">
                ...
                <p class="text-xs text-muted-foreground mt-1 max-w-[200px]">Devices must be in the same Area and POP as this server.</p>
              </div>
              ...
              <div v-else class="space-y-6">
                ...
                <!-- Alert Banner -->
                <div
                  v-if="isOverlapDetected"
                  class="flex gap-3 bg-destructive/10 border border-destructive/20 p-3 rounded-lg animate-in fade-in slide-in-from-top-1">
