  ...
  <AppLayout
    ...
    <div class="flex flex-col gap-6 p-4 md:p-6">
      ...
      <div class="grid gap-6 md:grid-cols-2">
        <!-- Map Card -->
        <Card v-if="area.boundary" class="overflow-hidden shadow-sm md:col-span-2">
          <CardHeader class="border-b pb-2">
            <div class="flex items-center gap-2">
              <MapIcon class="h-5 w-5 text-primary" />
              <CardTitle>Wilayah</CardTitle>
            </div>
            <CardDescription>Peta wilayah cakupan infrastruktur ini.</CardDescription>
          </CardHeader>
          <CardContent class="p-0">
            <div ref="mapContainer" class="z-0 h-[400px] w-full"></div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent class="space-y-4">
            <div class="grid grid-cols-2 gap-y-3 text-sm">
              <div class="text-muted-foreground">Kode Wilayah</div>
        ...
        <Card>
          ...
          <CardContent class="space-y-4">
            <div class="grid grid-cols-2 gap-y-3 text-sm">
              ...
              <div class="text-muted-foreground">Kota/Kabupaten</div>
              <div>{{ regencyName }}</div>

              <div class="text-muted-foreground">Kecamatan</div>
              <div>{{ districtName }}</div>

              <div class="text-muted-foreground">Kelurahan/Desa</div>
              <div>{{ villageName }}</div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Jumlah infrastruktur yang terdaftar di wilayah ini.</CardDescription>
        </CardHeader>
        <CardContent>
          <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
            <div class="flex flex-col items-center justify-center rounded-xl border bg-card p-6 transition-colors hover:bg-muted/50">
              <Building2 class="mb-2 h-10 w-10 text-blue-500" />
              <span class="text-3xl font-bold tracking-tight">{{ stats.pops_count }}</span>
              <span class="mt-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">Point of Presence</span>
            </div>
            <div class="flex flex-col items-center justify-center rounded-xl border bg-card p-6 transition-colors hover:bg-muted/50">
              <Server class="mb-2 h-10 w-10 text-green-500" />
              <span class="text-3xl font-bold tracking-tight">{{ stats.servers_count }}</span>
              <span class="mt-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">Server Room</span>
            </div>
            <div class="flex flex-col items-center justify-center rounded-xl border bg-card p-6 transition-colors hover:bg-muted/50">
              <Cpu class="mb-2 h-10 w-10 text-purple-500" />
              <span class="text-3xl font-bold tracking-tight">{{ stats.olts_count }}</span>
              <span class="mt-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">Active Devices</span>
            </div>
            <div class="flex flex-col items-center justify-center rounded-xl border bg-card p-6 transition-colors hover:bg-muted/50">
              <Box class="mb-2 h-10 w-10 text-orange-500" />
              <span class="text-3xl font-bold tracking-tight">{{ stats.odps_count }}</span>
              <span class="mt-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">Passive Devices</span>
            </div>
            <div class="flex flex-col items-center justify-center rounded-xl border bg-card p-6 transition-colors hover:bg-muted/50">
              <Monitor class="mb-2 h-10 w-10 text-cyan-500" />
              <span class="text-3xl font-bold tracking-tight">{{ stats.cpes_count }}</span>
              <span class="mt-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">CPE / Customer</span>
            </div>
          </div>
        </CardContent>
      </Card>
