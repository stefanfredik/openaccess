<script setup lang="ts">
  ...
  const variantClasses = {
    ...
    primary: 'border-primary/20 bg-primary/5 text-primary dark:text-foreground',
  ...
  const iconClasses = {
    ...
    primary: 'text-primary dark:text-foreground',
...
<template>
  <Card
    ...
    <div class="absolute top-0 right-0 p-3 opacity-10 transition-opacity group-hover:opacity-20">
    ...
    <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <component :is="icon" v-if="icon" class="h-4 w-4 transition-transform group-hover:scale-110" :class="iconClasses[variant || 'default']" />
    </CardHeader>
    <CardContent>
