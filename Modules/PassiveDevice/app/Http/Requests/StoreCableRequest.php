    ...
    public function rules(): array
    {
        return [
            ...
            'code' => [
                'required',
                'string',
                \Illuminate\Validation\Rule::unique('pd_cables')->where(fn($query) => $query->where('company_id', auth()->user()->company_id)),
            ],
            'name' => ['required', 'string', 'max:255'],
            'length' => ['required', 'numeric', 'min:0'],
            'core_count' => ['required', 'integer', 'min:0'],
            'type' => ['required', \Illuminate\Validation\Rule::in(['Single Mode', 'Multi Mode'])],
