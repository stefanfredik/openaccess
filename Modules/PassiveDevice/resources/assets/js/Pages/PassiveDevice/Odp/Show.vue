            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <Button variant="ghost" size="icon" as-child>
                        <Link :href="odpIndex().url">
                            <ArrowLeft class="h-4 w-4" />
                        </Link>
                    </Button>
                    <div>
                        <h1 class="text-2xl font-bold tracking-tight">{{ odp.name }}</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <CardContent>
                        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
                            <div>
                                ...
                                <dd class="text-sm font-semibold">{{ odp.area?.name || '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Status</dt>
                                <dd>
                                    <Badge :variant="odp.is_active ? 'default' : 'destructive'">
                                        {{ odp.is_active ? 'Active' : 'Inactive' }}
                                    </Badge>
                                </dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Splitter Type</dt>
                                <dd class="text-sm font-semibold">{{ odp.splitter_type || '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Ports (Used/Total)</dt>
                                <dd class="text-sm font-semibold">{{ odp.used_port_count }} / {{ odp.port_count }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Brand / Model</dt>
                                <dd class="text-sm font-semibold">{{ odp.brand || '-' }} / {{ odp.model || '-' }}</dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd class="text-sm font-semibold">{{ odp.installed_at || '-' }}</dd>
                            </div>
                            <div class="sm:col-span-2">
                                <dt class="text-sm font-medium text-muted-foreground">Description</dt>
                                <dd class="text-sm mt-1">{{ odp.description || 'No description provided.' }}</dd>
                            </div>
                        </dl>
                    </CardContent>
                </Card>
                ...
                <div class="space-y-6">
                    <Card>
                        ...
                        <CardContent>
                            <div v-if="odp.photo" class="aspect-square relative rounded-md overflow-hidden bg-muted">
                                <img :src="odp.photo" :alt="odp.name" class="object-cover w-full h-full" />
                            </div>
