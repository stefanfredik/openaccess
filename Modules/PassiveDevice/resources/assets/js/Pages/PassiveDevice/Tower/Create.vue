            ...
            <form @submit.prevent="submit">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Height and construction details of the tower.
                        </CardDescription>
                    </CardHeader>
                    <CardContent class="grid gap-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <Input id="code" v-model="form.code" :class="{ 'border-destructive': form.errors.code }" placeholder="TOWER-XYZ" />
                        ...
                        <div class="space-y-2">
                            ...
                            <Input id="name" v-model="form.name" :class="{ 'border-destructive': form.errors.name }" placeholder="Tower Site A" />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <Input id="height" type="number" v-model="form.height" :class="{ 'border-destructive': form.errors.height }" />
                                <p v-if="form.errors.height" class="text-sm text-destructive">{{ form.errors.height }}</p>
                            </div>
                            <div class="space-y-2">
                                <Label for="segment_count">Segment Count</Label>
                                <Input id="segment_count" type="number" v-model="form.segment_count" :class="{ 'border-destructive': form.errors.segment_count }" />
                                <p v-if="form.errors.segment_count" class="text-sm text-destructive">{{ form.errors.segment_count }}</p>
                            </div>
                        </div>
                    ...
                    <CardFooter class="flex justify-end gap-2 border-t p-6 mt-6">
                        <Button variant="outline" as-child>
                            <Link :href="towerIndex().url">Cancel</Link>
                        </Button>
