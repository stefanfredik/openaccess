<script setup lang="ts">
  ...
  const form = useForm({
    infrastructure_area_id: props.odf.infrastructure_area_id.toString(),
    code: props.odf.code,
    name: props.odf.name,
    port_count: props.odf.port_count,
    used_port_count: props.odf.used_port_count,
    core_capacity: props.odf.core_capacity,
    brand: props.odf.brand || '',
    model: props.odf.model || '',
    longitude: props.odf.longitude || '',
    latitude: props.odf.latitude || '',
    description: props.odf.description || '',
    is_active: !!props.odf.is_active,
    installed_at: props.odf.installed_at || '',
  })
  ...
  <AppLayout
    ...
    <div class="mx-auto flex max-w-4xl flex-col gap-6 p-4 md:p-6 lg:p-8">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Edit ODF</h1>
          <p class="text-muted-foreground">Update Optical Distribution Frame device details.</p>
        </div>
      </div>
      ...
      <form @submit.prevent="submit">
        <Card>
          <CardHeader>
            <CardTitle>ODF Information</CardTitle>
            <CardDescription>
              Update specifications and location for
              {{ odf.name }}.
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter class="mt-6 flex justify-end gap-2 border-t p-6">
            <Button variant="outline" as-child>
              <Link :href="route('passive-device.odf.index')">Cancel</Link>
            </Button>
