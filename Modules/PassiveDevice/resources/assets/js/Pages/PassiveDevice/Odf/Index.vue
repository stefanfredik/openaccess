...
<template>
  ...
  <AppLayout :breadcrumbs="[{ title: 'ODFs', href: route('passive-device.odf.index') }]">
    <div class="flex flex-col gap-6 p-4 md:p-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">ODFs</h1>
          <p class="text-muted-foreground">Manage Optical Distribution Frame (ODF) infrastructure.</p>
        </div>
        <Button as-child>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription> List of all ODF devices. </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow v-for="odf in odfs.data" :key="odf.id">
                <TableCell class="font-medium">{{ odf.code || '-' }}</TableCell>
                <TableCell>{{ odf.name }}</TableCell>
                <TableCell>{{ odf.area?.name || '-' }}</TableCell>
                <TableCell>
                  {{ odf.used_port_count }} /
                  {{ odf.port_count }}
                </TableCell>
                <TableCell>
                  <Badge :variant="odf.is_active ? 'default' : 'destructive'">
                    {{ odf.is_active ? 'Active' : 'Inactive' }}
                  </Badge>
                </TableCell>
              ...
              <TableRow v-if="odfs.data.length === 0">
                <TableCell colspan="6" class="h-24 text-center"> No ODFs found. </TableCell>
              </TableRow>
            </TableBody>
          </Table>
        </CardContent>
