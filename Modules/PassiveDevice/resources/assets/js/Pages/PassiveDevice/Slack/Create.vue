...
import { index as slackIndex, store as slackStore } from '@/routes/passive-device/slack';
...
const submit = () => {
    form.post(slackStore().url);
};
</script>

<template>
    <Head title="Add Slack" />

    <AppLayout :breadcrumbs="[
        { title: 'Slacks', href: slackIndex().url },
        { title: 'Add Slack', href: '#' }
    ]">
        <div class="flex flex-col gap-6 p-4 md:p-6 lg:p-8 max-w-4xl mx-auto">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Add Slack</h1>
                    <p class="text-muted-foreground">Register a new Slack storage coil asset.</p>
                </div>
            </div>
            ...
            <form @submit.prevent="submit">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Details about the fiber storage coil.
                        </CardDescription>
                    </CardHeader>
                    <CardContent class="grid gap-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <Input id="code" v-model="form.code" :class="{ 'border-destructive': form.errors.code }" placeholder="SLACK-XYZ" />
                        ...
                        <div class="space-y-2">
                            ...
                            <Input id="name" v-model="form.name" :class="{ 'border-destructive': form.errors.name }" placeholder="Slack Coil Site A" />
                    ...
                    <CardFooter class="flex justify-end gap-2 border-t p-6 mt-6">
                        ...
                        <Button type="submit" :disabled="form.processing">
                            {{ form.processing ? 'Saving...' : 'Save Slack' }}
                        </Button>
                    </CardFooter>
                </Card>
            </form>
