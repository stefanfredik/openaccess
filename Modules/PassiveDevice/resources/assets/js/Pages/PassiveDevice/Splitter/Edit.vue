<script setup lang="ts">
  ...
  // import { index as splitterIndex, update as splitterUpdate } from '@/routes/passive-device/splitter';
  ...
  const form = useForm({
    infrastructure_area_id: props.splitter.infrastructure_area_id.toString(),
    code: props.splitter.code,
    name: props.splitter.name,
    ratio: props.splitter.ratio || '',
    loss_db: props.splitter.loss_db || '',
    brand: props.splitter.brand || '',
    model: props.splitter.model || '',
    longitude: props.splitter.longitude || '',
    latitude: props.splitter.latitude || '',
    description: props.splitter.description || '',
    is_active: !!props.splitter.is_active,
    installed_at: props.splitter.installed_at || '',
  })

  const submit = () => {
    form.put(route('passive-device.splitter.update', props.splitter.id))
  }
</script>

<template>
  <Head title="Edit Splitter" />
  ...
  <AppLayout
    :breadcrumbs="[
      ...
      { title: 'Edit Splitter', href: '#' },
    ]">
    <div class="mx-auto flex max-w-4xl flex-col gap-6 p-4 md:p-6 lg:p-8">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold tracking-tight">Edit Splitter</h1>
          <p class="text-muted-foreground">Update PLC/FBT Optical Splitter details.</p>
        </div>
      </div>
      ...
      <form @submit.prevent="submit">
        <Card>
          <CardHeader>
            ...
            <CardDescription> Update specifications for {{ splitter.name }}. </CardDescription>
          </CardHeader>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div class="space-y-2">
                ...
                <Input
                  ...
                  v-model="form.ratio"
                  :class="{
                    'border-destructive': form.errors.ratio,
                  }" />
                <p v-if="form.errors.ratio" class="text-sm text-destructive">
                  {{ form.errors.ratio }}
                </p>
              </div>
              <div class="space-y-2">
                ...
                <Input
                  id="loss_db"
                  v-model="form.loss_db"
                  :class="{
                    'border-destructive': form.errors.loss_db,
                  }" />
                <p v-if="form.errors.loss_db" class="text-sm text-destructive">
                  {{ form.errors.loss_db }}
                </p>
              </div>
            </div>
          ...
          <CardFooter class="mt-6 flex justify-end gap-2 border-t p-6">
            <Button variant="outline" as-child>
              <Link :href="route('passive-device.splitter.index')">Cancel</Link>
            </Button>
            <Button type="submit" :disabled="form.processing">
              {{ form.processing ? 'Updating...' : 'Update Splitter' }}
            </Button>
          </CardFooter>
        </Card>
      </form>
