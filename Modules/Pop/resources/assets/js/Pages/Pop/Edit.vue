...
const form = useForm({
    ...
    electrical_capacity: props.pop.electrical_capacity ?? undefined,
...
<template>
    ...
    <AppLayout
        :breadcrumbs="[
            ...
            { title: 'Ubah', href: popEdit({ pop: pop.id }).url },
        ]"
    >
        <div class="mx-4 max-w-4xl space-y-6 p-4 md:p-4">
            <!-- Header -->
            <div class="space-y-1">
                <h1 class="text-2xl font-bold tracking-tight">Ubah Data POP</h1>
                <p class="text-sm text-foreground/60 text-muted-foreground">
                    Perbarui informasi Point of Presence (POP) yang sudah ada.
                </p>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <Card class="border shadow-none">
                    <CardHeader class="pb-4">
                        ...
                        <CardDescription class="text-xs"
                            >Detail identitas dan klasifikasi
                            POP.</CardDescription
                        >
                    </CardHeader>
                ...
                <Card class="border shadow-none">
                    <CardHeader class="pb-4">
                        <CardTitle class="text-base font-semibold"
                            >Infrastruktur Daya</CardTitle
                        >
                        <CardDescription class="text-xs"
                            >Informasi kapasitas dan sumber daya
                            listrik.</CardDescription
                        >
                    </CardHeader>
                    <CardContent class="space-y-5">
                        <div class="space-y-1.5">
                            <Label
                                for="electrical_capacity"
                                class="text-sm font-medium"
                                >Kapasitas Listrik (VA/Watt)</Label
                            >
                            <Input
                        ...
                        <div class="flex items-center space-x-2 pt-2">
                            <Switch
                                ...
                                @update:checked="form.has_backup_power = $event"
                            />
                            <Label
                                for="backup-power"
                                class="text-sm font-medium"
                                >Memiliki Daya Cadangan?</Label
                            >
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card class="border shadow-none">
                    <CardHeader class="pb-4">
                        <CardTitle class="text-base font-semibold"
                            >Detail Lokasi</CardTitle
                        >
                        <CardDescription class="text-xs"
                            >Koordinat dan alamat fisik POP.</CardDescription
                        >
                    </CardHeader>
                    <CardContent class="space-y-5">
                        <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="space-y-1.5">
                                <Label for="regency" class="text-sm font-medium"
                ...
                <Card class="border shadow-none">
                    <CardHeader class="pb-4">
                        ...
                        >
                        <CardDescription class="text-xs"
                            >Foto lokasi dan catatan tambahan.</CardDescription
                        >
                    </CardHeader>
                    <CardContent class="space-y-5">
                        <div class="space-y-1.5">
                            ...
                            <!-- Existing Photos -->
                            <div
                                v-if="pop.photos && pop.photos.length > 0"
                                class="mb-4 grid grid-cols-3 gap-4 md:grid-cols-4"
                            >
                                <div
                                    v-for="photo in pop.photos"
                                    :key="photo.id"
                                    class="group relative aspect-square overflow-hidden rounded-lg border border-slate-100 shadow-sm"
                                >
                                    <img
                                        :src="'/storage/' + photo.path"
                                        class="h-full w-full object-cover transition-transform group-hover:scale-105"
                                    />
                                    <!-- Optional: Add delete button for individual photos if backend supports it -->
                                </div>
                            </div>
