...
<template>
    ...
    >
        <div
            ...
            @click="triggerBrowse"
            :class="cn(
                'relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer transition-colors duration-200 bg-white dark:bg-zinc-950',
        ...
        >
            ...
            <!-- Preview State -->
            <div v-if="previewUrl || modelValue" class="relative w-full h-full p-2 flex items-center justify-center group">
                <img 
                    v-if="previewUrl" 
                    :src="previewUrl" 
                ...
                <div v-else class="flex flex-col items-center">
                    <UploadCloud class="w-10 h-10 text-emerald-500 mb-2" />
            ...
            <div v-else class="flex flex-col items-center justify-center pt-5 pb-6 text-center">
                ...
                <p class="mb-2 text-sm text-gray-700 dark:text-gray-300">
                    <span class="font-semibold text-gray-900 dark:text-gray-100">Upload File</span> Or <span class="text-emerald-500 font-semibold">Browse</span>
                </p>
                <p v-if="description" class="text-xs text-gray-500 dark:text-gray-400">
                    {{ description }}
                </p>
                <p v-else class="text-xs text-gray-500 dark:text-gray-400">
                    Format: PNG, JPG, PDF & Max file size: {{ maxSize || 5 }} MB
                </p>
            </div>
        </div>
        <p v-if="error" class="mt-2 text-sm text-destructive font-medium">
            {{ error }}
        </p>
    </div>
</template>
