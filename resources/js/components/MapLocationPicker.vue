<script setup lang="ts">
  ...
  const drawAreaBoundary = (id: string | number) => {
    ...
    const area = props.areas.find((a: any) => a.id.toString() === id.toString())
  ...
  <div class="flex flex-col gap-2">
    ...
    <div class="flex items-center gap-3">
      <div class="grid grow grid-cols-2 gap-2">
        <div class="relative">
          <span class="absolute top-1/2 left-3 -translate-y-1/2 text-[10px] font-bold text-slate-400 uppercase">Lat</span>
          <div class="border-borderpx-3 flex h-11 items-center rounded-lg border pl-10 font-mono text-sm text-slate-600">
            {{ latitude || '-' }}
          </div>
        </div>
        <div class="relative">
          <span class="absolute top-1/2 left-3 -translate-y-1/2 text-[10px] font-bold text-slate-400 uppercase">Lng</span>
          <div class="border-borderpx-3 flex h-11 items-center rounded-lg border pl-10 font-mono text-sm text-slate-600">
            {{ longitude || '-' }}
          </div>
        </div>
      ...
      <Dialog v-model:open="isOpen">
        <DialogTrigger as-child>
          <Button variant="outline" class="h-11 gap-2 border-primary/20 px-4 text-primary hover:bg-primary/5 hover:text-primary">
        ...
        <DialogContent class="gap-0 overflow-hidden border-none p-0 shadow-2xl sm:max-w-[700px]">
          <DialogHeader class="border-b bg-white p-6 pb-4">
            ...
            <DialogDescription> Klik pada peta atau geser penanda untuk menetapkan koordinat lokasi. </DialogDescription>
          </DialogHeader>
          ...
          <div class="relative h-[450px] bg-slate-100">
            ...
            <div class="absolute bottom-6 left-1/2 z-[40] flex -translate-x-1/2 gap-3">
              <div class="flex items-center gap-4 rounded-full border bg-white/90 px-5 py-2.5 shadow-xl backdrop-blur-md">
                <div class="flex flex-col items-center">
                  <span class="mb-1 text-[9px] leading-none font-bold text-slate-400 uppercase">Latitude</span>
                  <span class="font-mono text-sm leading-none text-slate-700">{{ tempLat || '0.000000' }}</span>
                </div>
                <div class="h-6 w-px bg-slate-200"></div>
                <div class="flex flex-col items-center">
                  <span class="mb-1 text-[9px] leading-none font-bold text-slate-400 uppercase">Longitude</span>
                  <span class="font-mono text-sm leading-none text-slate-700">{{ tempLng || '0.000000' }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-3 border-t bg-slate-50 p-4">
            ...
            <Button class="gap-2 px-8" @click="handleSave" :disabled="!tempLat || !tempLng">
              <Save class="h-4 w-4" />
              Simpan Lokasi
            </Button>
          </div>
